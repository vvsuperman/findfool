...
OJApp.controller('commonsetting', [ '$scope', '$http', 'Data',
		function($scope, $http, Data) {
			...
			$scope.comSave = function() {
				...
				$scope.extrainfo = [];
				if ($scope.school) {
					$scope.extrainfo.push($scope.school)
				}
				;
				if ($scope.spcial) {
					$scope.extrainfo.push($scope.spcial)
				}
				;
				if ($scope.exp) {
					$scope.extrainfo.push($scope.exp)
				}
				;
				if ($scope.link) {
					$scope.extrainfo.push($scope.link)
				}
				;
				if (!$scope.zdy) {
					$scope.testtime = 70
				...
				$http({
					url : WEBROOT + "/test/manage/setting/set",
					method : 'POST',
					headers : {
					...
					data : {
						...
						"extrainfo" : $scope.extrainfo.toString(),
						"emails" : $scope.emails
					}
