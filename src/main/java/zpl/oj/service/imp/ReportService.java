...
//		if(testid!=parentQuizId){
//		invites=inviteDao.getInviteByParentQuiz(parentQuizId);	
//			
//		}else if(testid==parentQuizId){
//		invites= inviteDao.getInviteByTid(invite.getTestid());
//		}
...
//		}
	 ...
	 * @param score
	 */
	private void sumScoreMap(Map<Integer,Integer> map, int id, int score){
		if(map.containsKey(id)){
			map.put(id, map.get(id)+score); //若包含，则+1
		}else{
			map.put(id, score); 					//若还没有，则设为1
		}
	}
	/*
	 * 根据试题集计算用户的做题维度
	...
	public Map getGroupDimension(Invite invite) {
		List<TuserProblem> tProblems = tuserProblemDao
				.findProblemByInviteId(invite.getIid());
		...
		for (TuserProblem tProblem : tProblems) {
			int setid = tProblem.getSetid();
			int groupId = 0;
			// 如果setid=0，就不能执行查询groupid，
			if (setid != ExamConstant.CUSTOM_SET_ID) {
				groupId = setDao.getGroupBySetid(setid);
			}

			// 选择题
			int level = tProblem.getLevel();
			if (setid != ExamConstant.CUSTOM_SET_ID
					&& tProblem.getType() == ExamConstant.OPTION) { // 选择题才比较。若为自定义试题，则不列入统计
				sumMap(setMap, setid);

				sumScoreMap(groupSetMap, groupId, tProblem.getScore());
				...
				if (tProblem.getRightanswer().equals(tProblem.getUseranswer())) {
					...
					sumScoreMap(groupMap, groupId, tProblem.getScore());

				}
				// 编程题
			} else if (setid != ExamConstant.CUSTOM_SET_ID
					&& tProblem.getType() == ExamConstant.PROGRAM) {
				// 编程题得分统计
				int solutionId = tProblem.getSolutionId();

				List<ResultInfo> results = tuserProblemDao
						.getProResult(solutionId);
				for (ResultInfo result : results) {
					// 总题目累加
					sumMap(setMap, setid);
					...
					if (result.getScore() > 0) {
						...
						sumScoreMap(groupMap, groupId, result.getScore());
					}
				}
			}
		...
		Map<String, Object> rtGroupSetMap = genGroupResultMap(groupSetMap,
				groupMap, 1);
		Map<String, Object> rtLevelMap = genResultMap(levelMap, rightLevelMap,
				2);
		Map<String, Object> rtMap = new HashMap<String, Object>();
		// rtMap.put("setRadar", rtSetMap);
		rtMap.put("setRadar", rtGroupSetMap);
	...
	private Map<String, Object> genGroupResultMap(
			Map<Integer, Integer> groupSetMap, Map<Integer, Integer> groupMap,
			int flag) {
		// 生成返回数据
		Map<String, Object> rtMap = new HashMap<String, Object>();
