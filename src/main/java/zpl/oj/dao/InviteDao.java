...
public interface InviteDao {
	...
	@Select("select IID,  TESTID,  HRID,  UID, begintime,  INVITETIME,  FINISHTIME,  SCORE,totalscore,STATE,DURATION"
			+ " FROM INVITE WHERE IID = #{0}")
	  Invite getInviteById(int iid);
	  
	@Insert(" INSERT INTO INVITE(  TESTID,  HRID,   UID,   INVITETIME,begintime,  FINISHTIME,  SCORE,totalscore,STATE,DURATION)"
			+ " VALUES (#{testid}, #{hrid}, #{uid}, #{invitetime},#{begintime}, #{finishtime}, #{score}, #{totalScore}, #{state}, #{duration})")
	...
	@Select("SELECT  t1.IID,  t1.TESTID, t1.begintime,  t1.HRID,   t1.UID,  t1.INVITETIME,  t1.FINISHTIME,  t1.SCORE,t1.totalscore,t1.STATE,t1.DURATION   "
			+ "  FROM INVITE t1, testuser t2 WHERE t1.testid=#{0} and t1.uid = t2.tuid and t2.email=#{1}")
	  Invite getInvites(int testid,String email);  
	
	
	@Select("SELECT  t1.IID,  t1.TESTID,   t1.HRID,   t1.UID,  t1.INVITETIME,t1.begintime,  t1.FINISHTIME,  t1.SCORE ,t1.STATE,t1.totalscore,t1.DURATION  "
