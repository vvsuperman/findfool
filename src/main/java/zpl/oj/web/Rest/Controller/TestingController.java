...
public class TestingController {
	...
	public ResponseBase login(@RequestBody Map<String, Object> params) {
		...
		if (tmpuser == null) {
			...
			// 逻辑有问题
			/*
			 * if(invite.getBegintime()!=null){ //用户已开始做题，直接返回tuserproblem的list
			 * List<TuserProblem> tuserProblems =
			 * tuserProblemDao.findProblemByInviteId(invite.getIid());
			 * rb.setState(1); rb.setMessage(tuserProblems); return rb; }else{
			 */
			// 未开始
			rb.setState(2);
			return rb;
...
*/
	...
	public ResponseBase startTest(@RequestBody Map<String, Object> params) {
		...
		// 建立定时器，到时间后将邀请置为无效
		new InviteReminder(Integer.parseInt(invite.getDuration()),
				invite.getIid(), inviteDao);
		inviteDao.updateInvite(invite);

		int inviteId = invite.getIid();
		List<TuserProblem> tProblems = tuserService.initialProblems(testid,
				tuid, inviteId);
		rb.setState(1);
	...
	/*
	 * 参数 testid email useranswer problemid nowProblemId 提交试题答案,并取下一道题
	 */
